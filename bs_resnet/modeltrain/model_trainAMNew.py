...
if __name__ == '__main__':
    ...
    # 训练模型
    model_struct = "LSTM"  # select LSTM or GRU
    if model_struct == "LSTM":
        ...
        hist = model.fit(x_train, y_train, batch_size=128, epochs=300, callbacks=[monitor],
                         validation_split=0.05)  # 128 
        model.save('../../webapp/model/AMLSTM.h5')
        df = pd.DataFrame.from_dict(hist.history)
        df.to_csv('../../webapp/model/AMLSTM_loss.csv', encoding='utf-8', index=False)
    ...
    # 装载模型以用于评估模型性能
    lstm = load_model('../../webapp/model/AMLSTM.h5', custom_objects={'TimeSeriesAttention': TimeSeriesAttention})
