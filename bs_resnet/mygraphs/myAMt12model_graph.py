...
def plot_step_3day(y_true, y_pred, test_dates, step_name, step_idx):
    ...
    ax.plot(x, y_pred_plot, label='LSTM Attention Prediction', linewidth=2, color='#ff7f0e')
    ...
    plt.title(f'LSTM Attention {step_name} Prediction (3 Days)', fontsize=20)
    ...
    # 保存图片
    save_name = f'LSTM_Attention_{step_name.replace("(","_").replace(")","")}_3Day.png'
    fig.tight_layout()
...
if __name__ == '__main__':
    ...
    # 4. 加载 LSTM_attention_final.h5 模型
    model_path = '../../webapp/model/LSTM_attention_final.h5'  # 固定为目标模型路径
    # 加载时传入自定义层，解决"Unknown layer: TimeSeriesAttention"报错
    model = load_model(model_path, custom_objects={'TimeSeriesAttention': TimeSeriesAttention})
    print(f"成功加载 LSTM Attention 模型：{model_path}")
